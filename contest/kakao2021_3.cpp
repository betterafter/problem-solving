...
vector<int> solution(vector<string> info, vector<string> query) {
    ...
    for(int p = 0; p < query.size(); p++){
        ...
        for(int i = 0; i < 3; i++){
            if(language(data[0]) != i && language(data[0]) != 3) continue;
            for(int j = 0; j < 2; j++){
                if(skill(data[1]) != j && skill(data[1]) != 2) continue;
                for(int k = 0; k < 2; k++){
                    if(career(data[2]) != k && career(data[2]) != 2) continue;
                    for(int l = 0; l < 2; l++){
                        if(food(data[3]) != l && food(data[3]) != 2) continue;
